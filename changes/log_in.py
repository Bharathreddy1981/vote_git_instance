...
def generate(db, final_data, jsondata):
    ...
    if len(final_data) == 0:
        ...
        password_check_final = password_encrypt.pass_check(final_data[0]["password"], passwd1)
        if  password_check_final == True:
            ...
            cursor = db.cursor()
            query = "UPDATE vote SET token = '" + str(token) + "' where email = '" + str(email1) + "' "
            ...
            query1 = "select * from vote where email='" + str(email1) + "'"
